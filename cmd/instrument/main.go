...
const (
	helperFile = "instrument_helper.go"
	fileSuffix = "_instrument.go"
	buildTag   = "instrumente"
)

func main() {
	processDir(".", func(f *ast.FuncDecl) bool { return f.Name.Name != "init" })
}
...
func processDir(path string, filter func(*ast.FuncDecl) bool) {
	...
	for fname, file := range pkg.Files {
		...
		astutil.AddImport(fs, file, "github.com/brownsys/tracing-framework-go/instrument")
		...
		fpath := filepath.Join(path, fname[:len(fname)-3]+fileSuffix)
...
package ` + pkgname + string(buf.Bytes())
	...
	if err = ioutil.WriteFile(filepath.Join(path, helperFile), b, 0664); err != nil {
